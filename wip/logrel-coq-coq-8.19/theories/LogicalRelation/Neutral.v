...
- Examples of as_apply usage can be seen in ty_app_ren, convneu_app_ren and complete_Pi lemmas
...
From LogRel.LogicalRelation Require Import NewApply.
...
Proof.
  intros.
  (** EXAMPLE 1 *)
    as_apply @ty_app. 8: eassumption. 5: apply H9.
    unfold Ren1_well_wk;
  (** EXAMPLE 2 *)   
    as_apply @ty_wk; gen_typing.
  
  (** Attempt without as_apply
...
Proof.
  intros.
  (** EXAMPLE 3 *)
    as_apply @convneu_app. 9: eassumption. 5: apply H9.
    unfold Ren1_well_wk;
  (** EXAMPLE 4 *)    
    as_apply @convneu_wk; gen_typing.

  (** Attempt without as_apply
  replace (cod[a .: ρ >> tRel]) with (cod⟨0 .: ρ >> ↑⟩[a..]) by (now bsimpl).
