...
Section GenericConsequences.
  ...
  Proof.
    intros.
    (** IMP: Var case we need to solve 
      B = ?X[t..] to directly apply convneu_app *)
   (* evar (k: nat -> nat).
    replace B with B⟨?k⟩[t..].
    eapply convneu_app.
    - admit.
    - admit.
    -*) 
  ...
  Proof.
    intros.
    (** IMP Var again we need to solve 
     A = ?X[t..] *)
  ...
  Proof.
    ...
    + eapply ty_simple_app.
      ...
      now eapply ty_var0.
    + cbn; now bsimpl (** asimpl suffices *).
     (** look at this case also later *)
    + now asimpl.
  Qed.
...
e34  Proof.
    intros ? ? Hf.
   (**  eapply ty_app;tea. 
       This won't work, see the error. Maybe no var case can also be a var case since eapply may not be that intelligent 

    - I want to see if the other solutions would work 
*)

    eapply typing_meta_conv.
    (** Var case B = ?X[tRel 0 .: ids]*)
    eapply ty_app; tea.
  ...
  Proof.
    ...
    - eapply @convtm_exp with (t' := t) (u' := t'); tea.
      ...
      2: eapply redtm_conv;  cbn ; [eapply redtm_meta_conv |..] ; [eapply redtm_beta |..]. (**  Var solve is there, also I am guess ?A = ?B[(tRel 0)..] is an unification problem because it ?A probably be the result of redtm_conv *)
