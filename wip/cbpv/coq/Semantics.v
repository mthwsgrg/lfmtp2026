...
(** Indirect defintions from original definition is commented out *)
...
(** 
- Four EXAMPLES where as_apply ideally works
- The preprocessing step qvar_to_evar is not working in Rocq 8.8.2 hence we specialize evar manually, and call one of the internal tactic to solve the goal 
*)

Lemma pstep_subst m n (C1 C2: comp m) (f: fin m -> value n):
  C1 ≽ C2 -> C1[f] ≽ C2[f].
Proof.
  intros H; inv H; asimpl.
  - constructor.
    
  - pose proof @pstepApp.
    evar (n1: nat).  evar (c1: comp (S ?n1)).  evar (v1: value ?n1).
    specialize (H ?n1 ?c1 ?v1).
    premises_to_subgoals H 0. 
  - constructor.
    destruct b; reflexivity.
  - pose proof @pstepLetin.
    evar (n1: nat).  evar (c1: comp (S ?n1)).  evar (v1: value ?n1).
    specialize (H ?n1 ?c1 ?v1).
    premises_to_subgoals H 0.     
  - pose proof @pstepCaseS.
    destruct b.
     + evar (n1: nat). evar (v1: value ?n1). evar (b1: bool). evar (c1': comp (S ?n1)). evar (c2': comp (S ?n1)). specialize (H ?n1 ?v1 ?b1 ?c1' ?c2').
       premises_to_subgoals H 0.
     + evar (n1: nat). evar (v1: value ?n1). evar (b1: bool). evar (c1': comp (S ?n1)). evar (c2': comp (S ?n1)). specialize (H ?n1 ?v1 ?b1 ?c1' ?c2').
       premises_to_subgoals H 0.
    
  - pose proof @pstepCaseP.
    evar (n1: nat). evar (v1': value ?n1).  evar (v2': value ?n1).  evar (c1' : comp (S (S ?n1))).
    specialize (H ?n1 ?v1' ?v2' ?c1').
    premises_to_subgoals H 0.    
Qed.
