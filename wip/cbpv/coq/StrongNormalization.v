...
Proof.
  ...
  - intros A ihA B ihB m n f []; cbn; eauto; intros H k g v Hv.
    ...
    asimpl.
    (* TODO-AS: Strange behaviour of __k *)
    now apply H.
Qed.
...
Section TypingLemmas.
  ...
  Proof.
    ...
    - assumption.
    - specialize (H1 _ _ (G_ext _ H)). asimpl in H1. eapply close_sn. (*
    - specialize (H2 _ _ (G_ext _ H)). revert H2. asimpl. apply close_sn.
    - intros v' Vv'. unfold beta. asimpl. apply H1. now apply G_scons.
    - intros v' Vv'. unfold beta. asimpl. apply H2. now apply G_scons.
  Qed. *)
      (*TODO-AS: Find a good solution. *)
  Admitted.
...
Theorem strong_normalization n (Gamma : ctx n) (c : comp n) (B : comptype) :
  Gamma ⊢ c : B -> sn c.
Proof.
