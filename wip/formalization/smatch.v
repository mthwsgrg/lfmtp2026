...
Lemma set_list_app_neq {A: Type} (Aeq_dec : forall (x y:A), {x = y} +{x<>y} ):  forall P a b, a<> b ->
                                                           set_add Aeq_dec b (a :: P) =
                                                             a :: (set_add Aeq_dec b P).
  intros.
  simpl.
...
Lemma set_list_app_eq {A: Type} (Aeq_dec : forall (x y:A), {x = y} +{x<>y} ):  forall P a,
                                                           set_add Aeq_dec a (a :: P) =
                                                             a :: P.
  intros.
  simpl.
...
Lemma set_union_assoc {A: Type} (Aeq_dec: forall (x y: A), {x=y} + {x <> y}): forall a X Y Z,  set_In a (set_union var_eqdec X (set_union var_eqdec Y Z)) <->
                                                                                   set_In a (set_union var_eqdec (set_union var_eqdec X Y) Z).
Proof.
  intros. split.
...
Lemma in_lhvar_implies_problem : forall P X, set_In X (lhvars_Probl P) -> set_In X (Problem_vars P).
Proof.
  intro P.
  induction P; intros.
  - now simpl in *.
...
Lemma problem_eqn_allvar_left_in_sexp : forall P σ τ X, set_In (equ_s σ τ) P ->
                                                set_In X (exp_vars_s σ) ->
                                                set_In X (Problem_vars P).
Proof.
  induction P; intros.
  - destruct H.
...
Lemma not_in_prob_allvar_not_in_eq_right : forall X P, ~set_In X (Problem_vars P) ->
                                             forall s t, set_In (equ s t) P ->
                                                    ~set_In X (exp_vars t).
Proof.
  intros.
  unfold not in *.
  intros.
  apply H. eapply problem_eqn_allvar_right_in; eauto.
Qed.


Lemma not_in_prob_allvar_not_in_eq_left_sexp : forall X P, ~set_In X (Problem_vars P) ->
                                               forall σ τ, set_In (equ_s σ τ) P ->
                                                     ~set_In X (exp_vars_s σ).
Proof.
  intros.
  unfold not in *.
  intros.
  apply H. eapply problem_eqn_allvar_left_in_sexp; eauto.
Qed.

Lemma not_in_prob_allvar_not_in_eq_right_sexp : forall X P, ~set_In X (Problem_vars P) ->
                                                forall σ τ, set_In (equ_s σ τ) P ->
                                                      ~set_In X (exp_vars_s τ).
Proof.
  intros.
  unfold not in *.
  intros.
  apply H. eapply problem_eqn_allvar_right_in_sexp; eauto.
Qed.
...
Lemma lhvars_X_desubst : forall P X0 X s, set_In X0 (lhvars_Probl (P |^^ ((X,s) :: nil)))
                                          -> X0 <> X -> 
                                          set_In X0 (lhvars_Probl P).
Proof.
  intro P.
  induction P; intros.
  - simpl. now simpl in H.
...
Lemma problem_vars_neq : forall P0 s t a, (equ s t <> a) -> Problem_vars (equ s t:: P0 |+ a) = Problem_vars (equ s t :: (P0 |+ a)).
Proof.
  intros.
  rewrite set_list_app_neq.
  reflexivity.
  assumption.
Qed.
...
Lemma lhvar_vars_add : forall P s t X, set_In X (lhvars_Probl (P |+ equ s t)) ->
                                  set_In X (set_union var_eqdec (lhvars_Probl P) (lhvars_Probl ((equ s t) :: nil))) .
...
Proof.
  ...
  - simpl in *.
    now apply set_union_intro2.
  - simpl in IHP. destruct a as [s' t' | σ τ].
    + destruct (Equation_eqdec (equ s t) (equ s' t')); subst.
      simpl. rewrite e in H. 
      rewrite set_list_app_eq in H. simpl in H.
      apply set_union_elim in H.
      destruct H.
      apply set_union_intro1. now apply set_union_intro1.
      apply set_union_intro1. now apply set_union_intro2.
      rewrite set_list_app_neq in H; eauto. simpl in H.
      simpl. eapply set_union_assoc. apply var_eqdec.
      apply set_union_elim in H.
      destruct H.
      now apply set_union_intro1.
      apply set_union_intro2. eapply IHP; eauto.
...
Lemma lhvar_vars_add_sexp : forall P σ τ X, set_In X (lhvars_Probl (P |+ equ_s σ τ)) ->
                                       set_In X (set_union var_eqdec (lhvars_Probl P) (lhvars_Probl ((equ_s σ τ) :: nil))) .
Proof.
  intro P.
  induction P; intros.
  - simpl in *.
...
Lemma lhvar_ext_vars_exp : forall P s t X, set_In X (lhvars_Probl (P |+ (equ s t))) ->
                                            (forall Y, set_In Y (exp_vars s) -> set_In Y (lhvars_Probl P)) ->
                                            set_In X (lhvars_Probl P).
Proof.
  intros.
...
Lemma lhvar_ext_vars_sexp : forall P σ τ X, set_In X (lhvars_Probl (P |+ (equ_s σ τ))) ->
                                            (forall Y, set_In Y (exp_vars_s σ) -> set_In Y (lhvars_Probl P)) ->
                                            set_In X (lhvars_Probl P).
Proof.
  intros.
  apply lhvar_vars_add_sexp in H.
  simpl in H.
  apply set_union_elim in H.
  destruct H; eauto.
Qed.  
...
Lemma problem_var_ext_vars_sexp : forall P σ τ X, set_In X (Problem_vars (P |+ (equ_s σ τ))) ->
                                            ~set_In X (exp_vars_s σ) -> ~ set_In X (exp_vars_s τ) ->
                                            set_In X (Problem_vars P).
Proof.
  intro P.
  induction P; intros.
...
Proof.
  ...
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    ...
    apply problem_var_ext_vars_exp in H1.
    apply problem_var_ext_vars_exp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left  with (s := App s t) (t:= App s' t') (X:=X)in H;eauto.
    simpl in H.
    intro. apply H.
    now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_right  with (s := App s t) (t:= App s' t') (X:=X)in H;eauto.
    simpl in H.
    intro. apply H.
    now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_left  with (s := App s t) (t:= App s' t') (X:=X)in H;eauto.
  ...
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    ...
    apply problem_var_ext_vars_exp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left with (s:= Lam s) (t:= Lam s') (X:=X) in H'; eauto.
    apply not_in_prob_allvar_not_in_eq_right with (s:= Lam s) (t:= Lam s') (X:=X) in H'; eauto.
    
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    intro.
    apply problem_var_remove_one_mem in H1.
    apply problem_var_ext_vars_sexp in H1.
    apply problem_var_ext_vars_exp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left with (s := s[σ]) (t:= s'[σ']) (X:=X) in H; eauto.
    intro.
    simpl in H. apply H. now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_right with (s := s[σ]) (t := s'[σ']) (X := X) in H; eauto.
    intro.
    simpl in H. apply H.  now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_left with (s := s[σ]) (t:= s'[σ']) (X:=X) in H; eauto.
    intro.
    simpl in H. apply H. now apply set_union_intro2.
    apply not_in_prob_allvar_not_in_eq_right with (s := s[σ]) (t := s'[σ']) (X := X) in H; eauto.
    intro.
    simpl in H. apply H.  now apply set_union_intro2.
  ...
  - apply set_nocommon_1_3_back with (X := X) in H'; eauto.
    ...
    apply problem_var_ext_vars_exp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left_sexp with (σ := s .:σ) (τ := s' .: σ') (X := X) in H; eauto.
    intro.
    apply H. simpl. now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_right_sexp with (σ := s .:σ) (τ := s' .: σ') (X := X) in H; eauto.
    intro.
    apply H. simpl. now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_left_sexp with (σ := s .:σ) (τ := s' .: σ') (X := X) in H; eauto.
    intro.
  ...
  - apply set_nocommon_1_3_back with (X := X) in H'; eauto.
    ...
    apply problem_var_ext_vars_sexp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left_sexp with (σ := σ >> τ) (τ := σ' >> τ') (X := X) in H; eauto.
    intro.
    apply H. simpl. now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_right_sexp with (σ := σ >> τ) (τ := σ' >> τ') (X := X) in H; eauto.
    intro.
    apply H. simpl. now apply set_union_intro1.
    apply not_in_prob_allvar_not_in_eq_left_sexp with (σ := σ >> τ) (τ := σ' >> τ') (X := X) in H; eauto.
    intro.
  ...
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    ...
    apply problem_var_ext_vars_exp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left with (s := Lam s [Zero .: σ >> ↑]) (t:= s'[σ']) in H';eauto. simpl in *.
    intro.
    apply H'.
    apply set_union_elim in H2.
    destruct H2. now apply set_union_intro1.
    apply set_union_intro2. now apply set_union_intro2.
    apply not_in_prob_allvar_not_in_eq_right with (s := Lam s [Zero .: σ >> ↑]) (t:= s'[σ']) in H';eauto.
  ...
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    ...
    apply problem_var_ext_vars_sexp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left_sexp with (σ := σ >> τ >> ρ) (τ := σ' >> τ') in H'; eauto.
    intro. apply H'. simpl.  simpl in H2.
    apply set_union_elim in H2.
    destruct H2.
    apply set_union_elim in H2.
    destruct H2.
    ...
    now apply set_union_intro1.
    apply set_union_intro2. now apply set_union_intro2.
    apply not_in_prob_allvar_not_in_eq_right_sexp with (σ := σ >> τ >> ρ) (τ := σ' >> τ') in H'; eauto.  
  ...
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    ...
    apply problem_var_ext_vars_sexp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left_sexp with (σ := s[σ] .: σ >> τ) (τ := σ' >> τ') in H'; eauto.
    intro. apply H'. simpl.  simpl in H2.
    apply set_union_elim in H2.
    destruct H2.
    apply set_union_elim in H2.
    ...
    now apply set_union_intro1.
    apply set_union_intro2. now apply set_union_intro2.
    apply not_in_prob_allvar_not_in_eq_right_sexp with (σ := s[σ] .: σ >> τ) (τ := σ' >> τ') in H'; eauto.
  ...
  - apply set_nocommon_1_3_back with (X:=X) in H'; eauto.
    ...
    apply problem_var_ext_vars_exp in H1.
    contradiction.
    apply not_in_prob_allvar_not_in_eq_left with (s := s[σ >> τ]) (t := s'[ρ]) in H'; eauto.
    intro. apply H'. simpl. simpl in H2.
    apply set_union_elim in H2.
    destruct H2.
    apply set_union_elim in H2.
    ...
    apply set_union_intro2.
    now apply set_union_intro2.
    apply not_in_prob_allvar_not_in_eq_right with (s := s[σ >> τ]) (t := s'[ρ]) in H'; eauto.
...
Proof.
  intros.
  destruct H; simpl in H0; apply set_nocommon_1_3;  intros.
  - simpl in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0. assumption.
  ...
  - simpl in H1.
    ...
    apply lhvar_ext_vars_exp in H1.
    apply lhvar_ext_vars_exp in H1.
    apply set_nocommon_1_3_back  with (X:=X) in H0; eauto.
    intros. apply problem_eqn_lhvar_in with (s:= App s t) (t:= App s' t'); eauto.
    simpl. now apply set_union_intro1.
    intros. apply problem_eqn_lhvar_in with (s:= App s t) (t:= App s' t'); eauto.
  ...
  - simpl in H1.
    ...
    apply lhvar_ext_vars_exp in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
    intros. eapply problem_eqn_lhvar_in; eauto. 
  - simpl in H1.
    apply problem_lhvar_remove_one_mem in H1.
    apply lhvar_ext_vars_sexp in H1.
    apply lhvar_ext_vars_exp in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
    intros. eapply problem_eqn_lhvar_in; eauto.
    simpl. now apply set_union_intro1.
  ...
  - simpl in H1.
    ...
    apply lhvar_ext_vars_exp in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
    intros. eapply problem_eqn_lhvar_in_sexp; eauto.
    simpl. now apply set_union_intro1.
    intros. apply problem_eqn_lhvar_in_sexp with (σ:= s .: σ) (τ:= s' .: σ'); eauto.
    apply set_add_intro. now right.
    simpl. now apply set_union_intro2.
  - simpl in H1.
    apply problem_lhvar_remove_one_mem in H1.
    ...
    intros. eapply problem_eqn_lhvar_in_sexp; eauto.
    simpl. now apply set_union_intro1.
    intros. apply problem_eqn_lhvar_in_sexp with (σ:= σ >> τ) (τ:= σ' >> τ'); eauto.
    apply set_add_intro. now right.
    simpl.  now apply set_union_intro2.
  - simpl in H1.
    apply problem_lhvar_remove_one_mem in H1.
    apply lhvar_ext_vars_exp in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
    intros. apply problem_eqn_lhvar_in with (s := Lam (s [Zero .: σ >> ↑])) (t := s'[σ']); eauto.
    simpl in *.
    apply set_union_elim in H2.
    destruct H2. now apply set_union_intro1.
    apply set_union_intro2. now apply set_union_intro2.
  ...
  - simpl in H1.
    ...
    apply lhvar_ext_vars_sexp in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
    intros. apply problem_eqn_lhvar_in_sexp with (σ := σ >> τ >> ρ) (τ := σ' >> τ'); eauto.
    simpl in *.
    eapply set_union_assoc. apply var_eqdec.
    assumption.
  - simpl in H1.
    apply problem_lhvar_remove_one_mem in H1.
    apply lhvar_ext_vars_sexp in H1.
    apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
    intros. apply problem_eqn_lhvar_in_sexp with (σ := s[σ] .: σ >> τ) (τ := σ' >> τ'); eauto.
    simpl in *.
    apply set_union_elim in H2.
    destruct H2.
    apply set_union_elim in H2.
  ...
  -  simpl in H1.
     ...
     apply lhvar_ext_vars_exp in H1.
     apply set_nocommon_1_3_back with (X:=X) in H0; eauto.
     intros. apply problem_eqn_lhvar_in with (s := s[σ >>τ]) (t := s'[ρ]); eauto.
     simpl in *.
     eapply set_union_assoc. apply var_eqdec.
     assumption.
  -  simpl in H3.
     destruct (var_eqdec X X0); subst.
     intro. apply in_lhvar_implies_problem in H3. revert H3.
     eapply problem_X_clear; repeat split; eauto.
     eapply not_in_prob_lhvar_not_in_eq_left; eauto.
     intro.  apply problem_lhvar_remove_one_mem in H3.
     contradiction.
     apply lhvars_X_desubst in H3; eauto.
     apply problem_lhvar_remove_one_mem in H3.
     apply set_nocommon_1_3_back with (X := X0) in H0; eauto.
Qed.
