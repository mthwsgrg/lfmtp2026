(** 
- Old solution of POPLMark B without as_apply
- EXAMPLE *, corresponds to the places where as_apply is used in the new development.
...
Lemma sub_weak1' {p:nat} (Delta : ctx p) A B C :
  SUB Delta |- A <: B -> SUB ((C .: Delta) >> (ren1 shift) ) |- A⟨shift⟩ <: B⟨shift⟩.
Proof. intros. eapply sub_weak';  eauto. Qed.



Lemma sub_weak {p q: nat} (Delta1: ctx p) (Delta2: ctx q) A1 A2 A1' A2' (xi:  fin p ->  fin q) :
  SUB Delta1 |- A1 <: A2 ->
 (forall x,  ren1 xi (Delta1 x) = Delta2 (xi x)) ->
  A1' = ren1 xi A1 -> A2' = ren1 xi A2 ->
  SUB Delta2 |- A1' <: A2' .
Proof.
  intros H. autorevert H. induction H using @sub_rec; intros; subst; asimpl; cbn; econstructor; eauto.
  - eapply IHsub2; try reflexivity.
...
Lemma sub_weak1 {p:nat} (Delta : ctx p) A A' B B' C :
  SUB Delta |- A <: B ->  A' = ren1 shift A ->  B' = ren1 shift B -> SUB ((C .: Delta) >> (ren1 shift) ) |- A' <: B'.
...
Proof with asimpl;eauto.
  ...
  - depind H... depind H1...
    ...
    + asimpl in *. eapply sub_narrow; try eapply H0.
      * destruct x; asimpl; cbn; eauto.
        (** EXAMPLE 1 -  here the sub_weak is the auxiliary lemma that is used *)
...
|  T_Tapp A B A' s B' : TY Δ;Γ |- s : all A B -> SUB Δ |- A' <: A -> B' = subst1 (A'..) B ->  TY Δ;Γ |- tapp s A' : B'
...
| E_Rec l ts t t' : EV t => t' -> In (l,t) ts -> EV (rectm ts) => rectm (update ts l t')
| E_LetL n P t t' s : EV t => t' -> EV (letpat n P t s) => (letpat n P t'  s)
...
Proof.
  ...
  - cbn. econstructor. apply IHty; eauto.
    ...
    (** EXAMPLE 3 *)
  - cbn. eapply T_Tapp with (A := ren1 ξ A) .
    3: asimpl.
  ...
  - cbn. asimpl.
    ...
    (** EXAMPLE 4 *)
    + substify. eapply pat_ty_subst; eauto.  
...
Lemma context_renaming_lemma' {q q' p p' : nat} (Δ: ctx q') (Γ: dctx p' q') (s: tm q p) A (ξ :  fin q ->  fin q') (ζ:  fin p ->  fin p') Δ' (Γ' : dctx p q):
  (forall x, ren1 ξ (Δ' x) = Δ (ξ x)) ->
  (forall (x:  fin p) , ren1 ξ (Γ' x) =  (Γ (ζ x))) ->
  TY Δ'; Γ' |- s : A -> forall s' A', s' = ren2 ξ ζ s -> A' = ren1 ξ A -> TY Δ; Γ |- s' : A'.
...
Proof.
   ...
   - constructor. eapply IHty; eauto.
     destruct x; asimpl; cbn; eauto; asimpl.
     (** EXAMPLE 5 *)
    +  assert (subst_ty σ (Γ f)  = ((Γ f)[σ]⟨id⟩)) as -> by (now asimpl).
       eapply context_renaming_lemma; eauto; now asimpl.
    + econstructor.     
    - constructor. apply IHty; eauto.
      + destruct x; asimpl; cbn; eauto.
        (** EXAMPLE 6 - sub_weak1 is auxiliary lemma*)
        * eauto.         
          specialize (eq1 f). 
          eapply sub_weak1 with (C := subst1 σ A) in eq1; eauto.  asimpl in eq1. eapply eq1.
      + intros x. asimpl.
      (** EXAMPLE 7 *)  
      assert ( subst1 ( σ >> (ren1 shift) ) (Γ x) =  ren1 shift (subst1 σ (Γ x)) ) by (now asimpl).
    ...
    (** EXAMPLE 8 *)    
    - eapply T_Tapp with (A := subst_ty σ A). 
      3: asimpl.
    asimpl in IHty. apply IHty; eauto.
    ...
    eapply pat_ty_subst; eauto.
      ...
      * intros x.
        ...
        -- eapply T_Var'. now asimpl.
           ...
           (** EXAMPLE 9 -  context_renaming_lemma' is an auxiliary lemma for this *)
...
Proof.
  ...
  - econstructor; eauto. asimpl. eapply IHhas_ty.
    destruct x; asimpl; cbn; eauto.
    + eapply sub_weak; try reflexivity. apply eq. intros y. now asimpl.
  - econstructor; eauto.

    (** EXAMPLE 10 *)
    replace A with (subst1 ids A) by (now asimpl). replace A' with (subst1 ids A') by (now asimpl).
...
Proof.
  ...
  - depind H_ty; [|eauto].
    + inv H_ty1; subst.
      (** EXAMPLE 12 *)
      * replace B with (subst1 ids B) by (now asimpl). 
      ...
      * pose proof (ty_inv_abs   _ _ _ _ _ _ _ H H0) as (?&?&?&?).
        ...
        (** EXAMPLE 13 *)
        replace x with (subst1 var_ty x) by (now asimpl).
        eapply context_morphism_lemma; eauto.
