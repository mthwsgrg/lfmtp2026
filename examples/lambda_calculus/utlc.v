(** Tactic code generation starts at line 231*)
...
Ltac  sigma_min gexp hexp :=
  ...
  |  lam (subst_tm ((scons) (var_tm (var_zero)) ((funcomp) (ren_tm (shift)) ?sigma0)) ?s0)  =>  first [ unify   (subst_tm sigma0 (lam s0)) (hexp)|  sigma_min   (subst_tm sigma0 (lam s0)) (hexp) ]
  |  app (ren_tm ?sigma0 ?s0) (ren_tm ?sigma0 ?s1)  =>  first [ unify   (ren_tm sigma0 (app s0 s1)) (hexp)|  sigma_min   (ren_tm sigma0 (app s0 s1)) (hexp) ]
  |  lam (ren_tm ((scons) (var_zero) ((funcomp) (shift) ?sigma0)) ?s0)  =>  first [ unify   (ren_tm sigma0 (lam s0)) (hexp)|  sigma_min   (ren_tm sigma0 (lam s0)) (hexp) ]
...
Ltac  heuristics gexp hexp :=
  (** Cases that handle renamify/substify conversion *)
  match   gexp  with
  ...
  (** Cases that handle specific equations if σ_μ function fails *)                    
...
(** match_concl_goal call the heuristics tactic for each sub-expressions gexp/hexp of a Predicate Pr *)
...
(** This pre-processing step turns premises into subgoals *)
...
(** Instantiates quantified variables in a hypothesis with existential variables of corresponding types *)
