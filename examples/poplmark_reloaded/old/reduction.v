From poplmark_reloaded.old Require Import stlc.
...
Proof.
  ...
  (** EXAMPLE 1,2,3 : step_beta', step_caseL' and step_caseR are auxiliary lemmas *)
...
Lemma mstep_subst m n (f g : fin m -> tm n) (s t : tm m) :
  ...
  mstep (s[f]) (t[g]).
Proof with eauto using @mstep, @step.
  intros st fg. apply mstep_trans with (t := t[f]); [eauto using mstep_inst|].
  ...
  - apply mstep_lam. apply IHt. intros [i|]; [|constructor].
    (** EXAMPLE 4, 5, 6 - here we have to substify manually *)
